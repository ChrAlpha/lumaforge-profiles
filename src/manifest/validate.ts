...
function issue(code: string, message: string, manifestPath?: string, entryId?: string, field?: string): ValidationIssue {
  return { code, message, manifestPath, entryId, field };
}

function addReleaseIssue(result: ValidationResult, release: boolean, validationIssue: ValidationIssue) {
  if (release) {
...
export async function validateProfiles(options: ValidateProfilesOptions): Promise<ValidationResult> {
  ...
  for (const manifestPath of manifestPaths) {
    if (seenManifestPaths.has(manifestPath)) {
      result.errors.push(issue("duplicate-manifest-path", "Manifest path is listed more than once.", manifestPath));
      continue;
    }
    ...
    try {
      ...
      result.errors.push(issue("json", error instanceof Error ? error.message : "Unable to read JSON.", manifestPath));
      continue;
    }

    const parsed = profileEntryManifestSchema.safeParse(raw);
    if (!parsed.success) {
      result.errors.push(issue("schema", formatZodError(parsed.error), manifestPath));
      continue;
    }
    ...
    if (previousPath) {
      result.errors.push(
        issue("duplicate-id", `Entry id duplicates ${previousPath}.`, manifestPath, manifest.id, "id")
      );
    } else {
    ...
    if (allowedFormats.length > 0 && !allowedFormats.includes(String(manifest.format))) {
      result.errors.push(
        issue(
          "kind-format",
    ...
    const expectedRole = ROLE_BY_FORMAT[String(manifest.format)];
    for (const asset of manifest.assets) {
      if (expectedRole && asset.role !== expectedRole) {
        result.errors.push(
          issue("kind-role", `format ${String(manifest.format)} expects role ${expectedRole}.`, manifestPath, manifest.id, "assets.role")
        );
      }

      if (!isSafeRelativePosixPath(asset.path)) {
        result.errors.push(issue("asset-path", `Asset path ${asset.path} escapes the entry directory.`, manifestPath, manifest.id, "assets.path"));
        continue;
      }
      ...
      if (actualHash !== asset.sha256.toLowerCase()) {
        result.errors.push(
          issue("asset-sha256", `Expected sha256 ${asset.sha256} but found ${actualHash}.`, manifestPath, manifest.id, "assets.sha256")
        );
      }
      ...
      if (!asset.sha256) {
        addReleaseIssue(
          ...
          issue("release-asset-sha256", "Asset sha256 is required for release.", manifestPath, manifest.id, "assets.sha256")
        );
      }
    }
    ...
    if (manifest.redistributionAllowed !== true) {
      addReleaseIssue(
        result,
        Boolean(options.release),
        issue("release-redistribution", "redistributionAllowed must be true for release.", manifestPath, manifest.id, "redistributionAllowed")
      );
    }
    if (!manifest.license.trim() || normalizedLicense === "NOASSERTION" || normalizedLicense === "UNLICENSED") {
      addReleaseIssue(
        result,
        Boolean(options.release),
        issue("release-license", "A redistributable release requires an explicit non-NOASSERTION license.", manifestPath, manifest.id, "license")
      );
    }
